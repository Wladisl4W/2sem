...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!preg_match('/^user[0-9]+$/', $_POST['login'])) {
        ...
        try {
            ...
            if ($row && password_verify($_POST['pass'], $row[1])) {
                session_start();
...
if (array_key_exists('register', $_GET)) {
    if (registered()) {
        ...
        try {
            ...
            $stmt->execute();
            $maxId = $stmt->fetch(PDO::FETCH_NUM)[0];
            $login = 'user' . ($maxId + 1);
            $password = randomPassword();
            $hash = password_hash($password, PASSWORD_DEFAULT);
            flash('Ваш логин: ' . htmlspecialchars($login, ENT_QUOTES, 'UTF-8') . '<br>Ваш пароль: ' . htmlspecialchars($password, ENT_QUOTES, 'UTF-8'));
            $stmt = $db->prepare("INSERT INTO users (id_user, login, pass) VALUES (0, ?, ?)");
...
<body>
    ...
    <form action="./login.php" method="post" class="px-2 maxw960 position-absolute top-50 translate-middle start-50">
        ...
        <div class="form-control d-flex">
            <a href="?register=true" class="btn-secondary btn m-1 w-50">Зарегистрироваться</a>
